...
#define BEGIN_SINGLE_THREAD_PART   \
    __syncthreads();               \
    if (FIRST_THREAD_IN_BLOCK()) { \
        do
#define END_SINGLE_THREAD_PART \
    while (0)                  \
        ;                      \
    }                          \
    __syncthreads()

#define BEGIN_SINGLE_THREAD  \
...
#define BEGIN_UNIQUE_THREAD_PART                    \
    __syncthreads();                                \
    if (FIRST_THREAD_IN_BLOCK() && FIRST_BLOCK()) { \
        do
#define END_UNIQUE_THREAD_PART \
    while (0)                  \
        ;                      \
    }                          \
    __syncthreads()
...
__forceinline__
    __device__ void
    memcpy_thread(volatile void* _dst, const volatile void* _src, uint size)
{
...
__forceinline__
    __device__ int
    strncpy_thread(volatile char* dst, volatile const char* src, size_t n)
{
    int i = 0;
    while (i < n && *src) {
        *dst++ = *src++;
...
struct LAST_SEMAPHORE {
    ...
    __device__ int is_last() volatile
    {
        ...
        if (ret == gridDim.x * gridDim.y * gridDim.z - 1)
            return 1;
        return 0;
    }
};
...
__device__ __forceinline__ void valcpy_256_interleave(double2* dst, double2* src)
{
    ...
    asm volatile("{.reg .f64 t1;\n\t"
                 ...
                 "st.wt.v2.f64 [%0],{t1,t2};\n\t"
                 "st.wt.v2.f64 [%1],{t3,t4};}"
                 :
                 : "l"(dst), "l"(d1), "l"(src), "l"(s1)
                 : "memory");
}

__device__ void __forceinline__ copy_block_large(char* dst, char* src, uint32_t len)
{
    int i = TID;
...
__device__ void __forceinline__ copy_block_16(char* dst, char* src, uint32_t len)
{
...
inline __device__ void copy_block(uchar* dst, volatile uchar* src, int size)
{
    int tid = TID;
    int newsize;
    // get the alignment
...
inline __device__ void copy_block_src_volatile(uchar* dst, volatile uchar* src, int size)
{
    int tid = TID;
    int newsize;
    // get the alignment
...
inline __device__ void copy_block_dst_volatile(volatile uchar* dst, uchar* src, int size)
{
    int tid = TID;
    int newsize;
    // get the alignment
