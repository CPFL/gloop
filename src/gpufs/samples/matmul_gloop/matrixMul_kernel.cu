...
matrixMulMain(int f_a, int f_b, int f_c, int wA, int wB, int perBlockX, int perBlockY, char n)
{
...
{
    gloop::open("mtx_a",O_GRDONLY, [=](int f_a) {
        if (f_a<0) { ERROR("Failed to open a");}

        gloop::open("mtx_b",O_GRDONLY, [=](int f_b) {
            if (f_b<0) { ERROR("Failed to open B");}

            char out[6]="mtx_c"; out[0]=n;
            gloop::open(out, O_GWRONLY, [=](int f_c) {
                if (f_c<0) { ERROR("Failed to open c");}

                matrixMulMain<BLOCK_SIZE>(f_a, f_b, f_c, wA, wB, perBlockX, perBlockY, n);
