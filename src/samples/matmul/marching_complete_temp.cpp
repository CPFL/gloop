...
performOuterLoop(gloop::DeviceLoop<>* loop, int wA, int wB, int perBlockX, int perBlockY, int f_a, int f_b, int f_c, int by)
{
    if (by == (loop->logicalBlockIdx().y+1)*perBlockY) {
...
{
    // for (int by=loop->logicalBlockIdx().y*perBlockY;by<(loop->logicalBlockIdx().y+1)*perBlockY;by++){
    int by=loop->logicalBlockIdx().y*perBlockY;
