...
void gpunet_microbench_init(GPUNETGlobals** pglobal, cudaStream_t* pkernel_stream) {
	...
	GLOOP_CUDA_SAFE_CALL(cudaDeviceSetLimit(cudaLimitStackSize, 16 * (1u << 10)));
	GLOOP_CUDA_SAFE_CALL(cudaDeviceSetLimit(cudaLimitMallocHeapSize, 3 * (1u << 30)));
...
int gpunet_server_init(struct sockaddr **paddr, struct sockaddr **dev_addr, int port) {
	...
	GLOOP_CUDA_SAFE_CALL(cudaHostAlloc((void**)&addr, sizeof(sockaddr), cudaHostAllocMapped));
	memset(addr, 0, sizeof(*addr));
	addr->sin_addr.s_addr = htonl(INADDR_ANY);
	addr->sin_port = ntohs(port);
	addr->sin_family = PF_INET;

	GLOOP_CUDA_SAFE_CALL(cudaHostGetDevicePointer((void**)dev_addr, addr, 0));
...
// called by (loop->logicalBlockIdx() 0) thread
__device__
...
// called by threads within non-zero loop->logicalBlockIdx()
__device__
void gpunet_wait_for_connection(int *sock, volatile int* tb_alloc_tbl, int nr_tb, volatile int *conn_finished) {
	...
	BEGIN_SINGLE_THREAD_PART {
		...
		while(!*conn_finished) {
			if (tb_alloc_tbl[loop->logicalBlockIdx().x] != -1) {
				ret = tb_alloc_tbl[loop->logicalBlockIdx().x];
				break;
			}
		}
...
void gpunet_finalize_connection(int sock, volatile int* tb_alloc_tbl) {
	BEGIN_SINGLE_THREAD_PART {
		tb_alloc_tbl[loop->logicalBlockIdx().x] = -1;
	} END_SINGLE_THREAD_PART;
}
#endif
