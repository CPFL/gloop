...
__device__ void performCopy(gloop::DeviceLoop<>* loop, uchar* scratch, int zfd, size_t me, size_t filesize, int trials, int ioSize, int loopCount, int count)
{
    volatile int res = 0;
    ...
    if (me < filesize) {
        ...
        gloop::fs::read(loop, zfd, me, toRead, scratch, [=](gloop::DeviceLoop<>* loop, int read) {
            ...
            performCopy(loop, scratch, zfd, me + GLOOP_SHARED_PAGE_SIZE * loop->logicalGridDim().x, filesize, trials, ioSize, loopCount, count + 1);
        });
        return;
    }
...
__device__ void gpuMain(gloop::DeviceLoop<>* loop, char* src, int trials, int ioSize, int loopCount)
{
    ...
    gloop::fs::open(loop, src, O_RDONLY, [=](gloop::DeviceLoop<>* loop, int zfd) {
        gloop::fs::fstat(loop, zfd, [=](gloop::DeviceLoop<>* loop, int filesize) {
            ...
            performCopy(loop, scratch, zfd, me, filesize, trials, ioSize, loopCount, 0);
        });
    });
}
