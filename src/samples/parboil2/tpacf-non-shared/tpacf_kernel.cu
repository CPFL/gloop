...
__global__ void gen_hists(hist_t* histograms, REAL* all_x_data, REAL* all_y_data,
    REAL* all_z_data, int NUM_SETS, int NUM_ELEMENTS)
{
    ...
    __shared__ struct cartesian* data_s;
    BEGIN_SINGLE_THREAD
    {
        data_s = new struct cartesian[BLOCK_SIZE];
    ...
    for (unsigned int i = 0; i < NUM_ELEMENTS; i += BLOCK_SIZE) {
        ...
        for (unsigned int j = (do_self ? i + 1 : 0); j < NUM_ELEMENTS;
            ...
            for (unsigned int k = 0;
                ...
                REAL distance = data_s[k].x * random_x_s + data_s[k].y * random_y_s + data_s[k].z * random_z_s;
