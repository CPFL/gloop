...
__device__ inline int gbench_recv_send_bw(int sock) {
  ...
  if (ret = gprepare_recv<LEN_MSG>(sock, g_message[loop->logicalBlockIdx().x])) {
  ...
  if (ret = gbench_recv<LEN_MSG, NR_ITERATION>(sock, g_message[loop->logicalBlockIdx().x])) {
  ...
  if (ret = gprepare_send<LEN_MSG>(sock, g_message[loop->logicalBlockIdx().x])) {
  ...
  if (ret = gbench_send<LEN_MSG, NR_ITERATION>(sock, g_message[loop->logicalBlockIdx().x])) {
...
__device__ inline int gbench_send_recv_lat(int sock) {
  ...
  for (int i = 0; i < NR_ITERATION; i++) {
    if (ret = gbench_send<LEN_MSG, 1>(sock, g_message[loop->logicalBlockIdx().x])) {
      printf("bench_send_recv bench_send ret: %d\n", ret);
      return ret;
    }
    
    if (ret = gbench_recv<LEN_MSG, 1>(sock, g_message[loop->logicalBlockIdx().x])) {
      printf("bench_send_recv bench_send ret: %d\n", ret);
      return ret;
    }
  }
