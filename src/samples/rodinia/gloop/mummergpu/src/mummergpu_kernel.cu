    ...
    const int min_match_len)
{
    ...
    int mustmatch = driver->mustmatch[threadIdx.x];
    int qry_match_len = driver->qry_match_len[threadIdx.x];
    int qrystart = driver->qrystart;
    ...
    if (!disabled) {
        ...
        result = MATCH_BASE(match_coords, qryid);
    ...
    for (;__syncthreads_or(qrystart <= last); qrystart++, result += RESULT_SPAN) {
    ...
    while (cur != badParent) {
        ...
        if (isLeaf || forceToParent) {
            ...
            if (isLeaf && isLeaf != queryflankingbase) {
                ...
                if ((left_in_ref != page_begin || page_shadow_left == -1) && (right_in_ref != page_end || page_shadow_right == -1)) {
